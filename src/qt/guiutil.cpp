...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Futurexco address (e.g. %1)").arg("XEBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
#endif
...
{
    // return if URI is not valid or is no Futurexco: URI
...
{
    // Convert futurexco:// to futurexco:
    //
    //    Cannot handle this later, because futurexco:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open futurexco.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Futurexco.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Futurexco.lnk
...
{
    return GetAutostartDir() / "futurexco.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a futurexco.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Futurexco\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Futurexco app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Futurexco app to startup item list
