namespace :replay do
  ...
  def replay(m, bln_arr)
    ...
    (bln_arr.size...arr_size).each do |i|
      price = Trade.with_currency('btcaud').where('created_at <= ?', Time.at(start.to_i + period * i)).last.price
      bln_arr[i] = (balances['aud'][i] + (balances['btc'][i] - 1) * price).to_f.round(2).to_s
    end
  ...
  def replay_student(m, bln_arr)
    ...
    (bln_arr.size...arr_size).each do |i|
      price = Trade.with_currency('btcaud').where('created_at <= ?', Time.at(start.to_i + period * i)).last.price
      bln_arr[i] = ((balances['aud'][i] + (balances['btc'][i] - 0.1) * price) * 10).to_f.round(2).to_s
    end
